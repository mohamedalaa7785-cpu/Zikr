...
const capacitorDeps = JSON.parse(read('mobile/capacitor-dependencies.json') || '{}');
...
for (const dependency of ['@capacitor/core', '@capacitor/android', '@capacitor/ios', '@capacitor/app']) {
  if (!capacitorDeps.dependencies?.[dependency] && !capacitorDeps.devDependencies?.[dependency]) {
    failures.push(`Mobile dependency manifest missing ${dependency}`);
  }
}
for (const host of ['*.supabase.co', 'accounts.google.com', 'api.alquran.cloud', 'hadithapi.com']) {
...
for (const token of ['zikrNative', 'appUrlOpen', 'networkStatusChange', 'Share', 'Camera', 'Filesystem', 'Haptics', 'scheduleLocalNotification']) {
