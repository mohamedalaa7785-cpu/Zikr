...
-- Ensure prerequisites exist when this migration is replayed on a fresh database
-- or run directly before the archived initial schema has created profiles.
...
drop policy if exists "prayer_locations_owner_all" on prayer_locations;
create policy "prayer_locations_owner_all" on prayer_locations
  for all using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- RLS Policies for prayer_preferences
drop policy if exists "prayer_preferences_owner_all" on prayer_preferences;
create policy "prayer_preferences_owner_all" on prayer_preferences
  for all using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- RLS Policies for prayer_notifications
drop policy if exists "prayer_notifications_owner_read" on prayer_notifications;
create policy "prayer_notifications_owner_read" on prayer_notifications
  for select using (auth.uid() = user_id);

drop policy if exists "prayer_notifications_owner_insert" on prayer_notifications;
...
drop trigger if exists update_prayer_locations_updated_at on prayer_locations;
