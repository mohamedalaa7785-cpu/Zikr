...
function isScholar(item: unknown): item is Scholar {
  if (!item || typeof item !== "object") return false;
  const scholar = item as Partial<Scholar>;
  return Boolean(
    scholar.id && scholar.slug && scholar.nameAr && scholar.nameEn
  );
}
...
export async function getAllScholars(): Promise<Scholar[]> {
  try {
    ...
    const validated = response.every(isScholar);
