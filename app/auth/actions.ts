 ...
 * Description: Authenticates via Supabase, sets HTTP-only session cookies, redirects to next path.
 * Tags: Auth
 *
 * Request (FormData):
 *   - email: string (required)
 *   - password: string (required)
 *   - next: string (optional, default: /profile)
 *
 * Responses:
 *   303: Redirect to next path with cookies set
