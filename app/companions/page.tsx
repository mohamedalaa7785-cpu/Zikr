    ...
    <Container className="py-12 space-y-12">
      <section className="text-center space-y-4">
        ...
        <p className="max-w-3xl mx-auto text-lg leading-8 arabic-muted text-pretty">
          سير أصحاب النبي محمد ﷺ المنشورة بعد مراجعتها في منصة ذِكر.
        </p>
      </section>
                  ...
                  <Card className="h-full flex flex-col hover:border-brand-gold/50 transition-colors cursor-pointer">
                    <div className="flex items-start gap-4 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h2 className="text-lg font-bold text-brand-gold leading-snug">{companion.name_ar}</h2>
                        {companion.title_ar && (
                          <p className="text-xs text-brand-gold/60 mt-0.5">{companion.title_ar}</p>
                        )}
                        {companion.name_en && (
                          <p className="text-xs text-brand-cream/40 mt-0.5" dir="ltr">{companion.name_en}</p>
                        )}
                      </div>
                    </div>
                    {companion.bio_ar && (
                      <p className="text-sm leading-relaxed arabic-muted line-clamp-3 flex-1">{companion.bio_ar}</p>
                    )}
                    <Badge variant="outline" className="mt-3 self-start text-xs">{category}</Badge>
                  </Card>
                </Link>
              ))}
            </div>
