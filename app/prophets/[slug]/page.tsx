...
export default function ProphetDetailPage() {
  ...
  if (!prophet) {
    return <Container className="py-12"><p className="text-center text-brand-cream/70">لم يتم العثور على النبي</p></Container>;
  }
    ...
    <Container className="py-12 space-y-8 max-w-4xl">
      {prophet.featured_image_url && (
        <div className="w-full h-96 rounded-lg overflow-hidden">
          <img src={prophet.featured_image_url} alt={prophet.name_ar} className="w-full h-full object-cover" />
        </div>
      )}

      <div className="text-center space-y-4">
        <h1 className="text-5xl font-bold text-brand-gold">{prophet.name_ar}</h1>
        {prophet.name_en && <p className="text-xl text-brand-cream/70">{prophet.name_en}</p>}
      </div>
      ...
      <Card className="p-8 space-y-6 bg-black/30 border-brand-gold/30">
        {prophet.bio_ar && (
          <div className="space-y-3">
            ...
            <p className="text-lg text-brand-cream/90 leading-relaxed" dir="rtl">{prophet.bio_ar}</p>
          </div>
        )}
